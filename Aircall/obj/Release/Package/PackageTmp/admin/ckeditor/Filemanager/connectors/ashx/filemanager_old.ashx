...
public class filemanager : IHttpHandler
{
    ...
    {
        if (path.Contains(System.Configuration.ConfigurationManager.AppSettings["SiteAddress"]))
        {
            path = path.Replace(System.Configuration.ConfigurationManager.AppSettings["SiteAddress"], "");
        }
        ...
        foreach (FileInfo fileInfo in RootDirInfo.GetFiles())
        {
            ...
            sb.AppendLine("\"Path\": \"" + System.Configuration.ConfigurationManager.AppSettings["SiteAddress"] + Path.Combine(path, fileInfo.Name) + "\",");
            sb.AppendLine("\"Filename\": \"" + fileInfo.Name + "\",");
            sb.AppendLine("\"File Type\": \"" + fileInfo.Extension.Replace(".", "") + "\",");

            if (IsImage(fileInfo))
            {
                sb.AppendLine("\"Preview\": \"" + System.Configuration.ConfigurationManager.AppSettings["SiteAddress"] + Path.Combine(path, fileInfo.Name) + "\",");
            }
            else
            {
    ...
    private string getInfo(string path)
    {
        ...
        else
        {
            ...
            sb.AppendLine("{");
            sb.AppendLine("\"Path\": \"" + System.Configuration.ConfigurationManager.AppSettings["SiteAddress"] + path + "\",");
            sb.AppendLine("\"Filename\": \"" + fileInfo.Name + "\",");
            sb.AppendLine("\"File Type\": \"" + fileInfo.Extension.Replace(".", "") + "\",");

            if (IsImage(fileInfo))
            {
                sb.AppendLine("\"Preview\": \"" + System.Configuration.ConfigurationManager.AppSettings["SiteAddress"] + path.Replace("~/","") + "\",");
            }
            else
            {
                sb.AppendLine("\"Preview\": \"" + String.Format("{0}{1}.png", IconDirectory, fileInfo.Extension.Replace(".", "")) + "\",");
    ...
    private string Rename(string path, string newName)
    {
        ...
        if ((attr & FileAttributes.Directory) == FileAttributes.Directory)
        {
            ...
            sb.AppendLine("\"Code\": 0,");
            sb.AppendLine("\"Old Path\": \"" + System.Configuration.ConfigurationManager.AppSettings["SiteAddress"] + path + "\",");
    ...
    private string AddFolder(string path, string NewFolder)
    {
        ...
        sb.AppendLine("{");
        sb.AppendLine("\"Parent\": \"" + System.Configuration.ConfigurationManager.AppSettings["SiteAddress"] + path + "\",");
