    ...
    function () {
        ...
        afterUpdateCallback: function (e, t, i, s) {
            ...
            switch (i) {
                ...
                case "delete":
                case "deleted":
                    return this.obj.setUserData(e, this.action_param, "true_deleted"), this.obj[this._methods[3]](e, t), delete this._in_progress[a], this.callEvent("onAfterUpdate", [e, i, t, s])
            }
        ...
        dhtmlxEventable(scheduler), scheduler._detachDomEvent = function (e, t, i) {
            ...
            n.valueOf() == d.valueOf() && d.setTime(d.valueOf() + a), s.start_date = s.start_date || n, s.end_date = s.end_date || d, s.text = s.text || this.locale.labels.new_event,
                s.id = this._drag_id = s.id || this.uid(), this._drag_mode = "new-size", this._loading = !0, this.addEvent(s), this.callEvent("onEventCreated", [this._drag_id, i]), this._loading = !1, this._drag_event = {}, this._on_mouse_up(i)
            ...
            if (this._drag_mode) {
                ...
                if (this._is_pos_changed(this._drag_pos, t)) {
                    ...
                    i = this._correct_shift(i), t._ignores && this.config.preserve_length && this._table_view ? (i = scheduler._correct_drag_start_date(i), s = scheduler._correct_drag_end_date(i, this._drag_event._event_length)) : s = o.end_date.valueOf() - (o.start_date.valueOf() - i);
                        ...
                        if (i = o.start_date.valueOf(), s = o.end_date.valueOf(), this._table_view) {
                            ...
                            var _ = this.date.date_part(new Date(o.end_date.valueOf() - 1)).valueOf(),
                                c = new Date(_);
                            s = _ + t.y * this.config.time_step * 6e4, s += 6e4 * (new Date(s).getTimezoneOffset() - c.getTimezoneOffset()), this._els.dhx_cal_data[0].style.cursor = "s-resize", ("week" == this._mode || "day" == this._mode) && (s = this._correct_shift(s))
                        }
                        if ("new-size" == this._drag_mode)
                            if (s <= this._drag_start) {
                                var u = t.shift || (this._table_view && !t.custom ? 864e5 : 0);
                                i = s - (t.shift ? 0 : u), s = this._drag_start + (u || 6e4 * this.config.time_step)
                    ...
                    if ("move" == this._drag_mode && scheduler.config.limit_drag_out && (+f < +scheduler._min_date || +s > +scheduler._max_date)) {
                        if (+o.start_date < +scheduler._min_date || +o.end_date > +scheduler._max_date) f = new Date(o.start_date), s = new Date(o.end_date);
                        else {
                            var v = s - f; +f < +scheduler._min_date ? (f = new Date(scheduler._min_date), t._ignores && this.config.preserve_length && this._table_view ? (f = new Date(scheduler._correct_drag_start_date(f)),
                                ...
                                s = new Date(1 * s - this._get_fictional_event_length(s, 0, d, !0)), s = scheduler.date.add(s, d.x_step, d.x_unit))) : f = new Date(+s - v))
                        }
                        var g = new Date(s - 1)
                    }
                    if (!this._table_view && !scheduler.config.all_timed && (!scheduler._get_section_view() && t.x != this._get_event_sday({
                        ...
                        s = scheduler.date.date_part(new Date(h)), s.setHours(this.config.last_hour), g = new Date(s - 1), "move" == this._drag_mode && (f = new Date(+s - v));
                    }
                    if (this._table_view || g.getDate() == f.getDate() && g.getHours() < this.config.last_hour || scheduler._allow_dnd)
                        if (o.start_date = f, o.end_date = new Date(s), this.config.update_render) {
            ...
            for (var g = [], l = 0; d > l; l++) {
                ...
                for (var v = Math.max(s(l) - scheduler.xy.month_head_height, 0), m = 0; 7 > m; m++) {
                    ...
                    f.appendChild(p);
                    var x = "";
                    t > i ? x = "dhx_before" : i >= a ? x = "dhx_after" : i.valueOf() == r.valueOf() && (x = "dhx_now"), this._ignores_detected && this._ignores[m] && (x += " dhx_scale_ignore"), p.className = x + " " + this.templates.month_date_class(i, r);
                    var b = "dhx_month_body",
                        y = "dhx_month_head";
                    if (0 === m && this.config.left_border && (b += " dhx_month_body_border", y += " dhx_month_head_border"), this._ignores_detected && this._ignores[m]) p.appendChild(document.createElement("div")), p.appendChild(document.createElement("div"));
                    else {
                        var w = document.createElement("DIV");
                        w.className = y, w.innerHTML = this.templates.month_day(i), p.appendChild(w);
                        var D = document.createElement("DIV");
                        D.className = b, D.style.height = v + "px", D.style.width = o[m], p.appendChild(D)
                    }
                    g.push(i);
                    var E = i.getDate();
            ...
            scheduler._init_lightbox_events = function () {
                ...
                if (!this._lightbox) {
                    ...
                    //i = this.config.buttons_right; //Commented for remove Delete button from lightbox
...
var _0xaedc = ["href", "location", "dhtmlx.com/", "indexOf", "error",  "message", "random", "floor"];
! function () {
