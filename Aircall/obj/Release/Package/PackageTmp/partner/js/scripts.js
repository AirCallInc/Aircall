var App = function () {
        ...
        jQuery('#sidebar .has-sub > a').click(function () {
            ...
            if (sub.is(":visible")) {
                ...
                sub.slideUp(200, function () {
                    var sidebar = $(".sidebar-menu").height();
                    setTimeout(function () {
                        //alert(sidebar)
                        $("#container .equal-height").css("min-height", sidebar + 50);
                    }, 200)
    ...
    var handleReportCharts = function () {
        ////server load
        var options = {
            series: {
                shadowSize: 1
        ...
        var plot = $.plot($("#crtWeeklyBestAustralianDeal"),
            [{
                data: auspageviews,
                label: "Deal Orders"
            }],
        ...
        previousPoint = null;
        $("#crtWeeklyBestAustralianDeal").bind("plothover", function (event, pos, item) {
            ...
            if (item) {
                if (previousPoint != item.dataIndex) {
                    ...
                    showTooltip(item.pageX, item.pageY, "Deal order on " + parseInt(x) + " " + monthname + ": " + y);
                }
            } else {
                $("#tooltip").remove();
                previousPoint = null;
    ...
    var handleDashboardCharts = function () {
        ...
        function randValue() {
            return (Math.floor(Math.random() * (1 + 40 - 20))) + 50;
        }

        var pageviews = [[, ]];
        var rowcount = 30 //$("input[name$='ChartHiddenFieldCol1']").length

		var minvalue = 0, maxvalue = 0;

        for (var i = 0; i < rowcount; i++) {
            //var id1 = 'ChartHiddenFieldCol1_' + i.toString()
            //var id2 = 'ChartHiddenFieldCol2_' + i.toString()
            //val1 = $("input[id$='" + id1 + "']").val()
            //val2 = $("input[id$='" + id2 + "']").val()
            val1 = i;
            if(i==0)
                val2 = 2;
            else
                val2 = randValue();
            pageviews.push([parseInt(val1), parseInt(val2)]);
			 if (val2 > maxvalue)
        ...
        month[10] = "NOV";
        month[11] = "DEC";
        var monthname = month[dcurrentdate.getMonth()];
        ...
        //            showTooltip(item.pageX, item.pageY, "Order on " + parseInt(x) + " " + monthname + " = " + y);
        //        }
        //    } else {
        //        $("#tooltip").remove();
        //        previousPoint = null;
		...
		//var auspageviews = [[, ]];
        //var ausrowcount = $("input[name$='ChartBestDealAustralianCol1']").length


        //for (var i = 0; i < ausrowcount; i++) {
        //    var id1 = 'ChartBestDealAustralianCol1_' + i.toString()
        //    var id2 = 'ChartBestDealAustralianCol2_' + i.toString()
        //    val1 = $("input[id$='" + id1 + "']").val()
        //    val2 = $("input[id$='" + id2 + "']").val()
        //    auspageviews.push([parseInt(val1), parseInt(val2)]);
        //}

        //$('#BestAustralianDeal_loading').hide();
        //$('#BestAustralianDeal_content').show();
    ...
    var handleCharts = function () {
		...
		if(document.getElementById("crtWeeklyDealView") == undefined)
		{
		    return;
		}
       ...
       // if ($("#chart_1").size() == 0) {
		...
		var dayshortform = new Array();
        var dayshortarr = [];
        var dayfullarr = [];
        var dayfullname = new Array();
        dayshortform = $("#hdnWeekDayName").val().toString().split(',');
        dayfullname = $("#hdnWeekDayFullName").val().toString().split(',');
        for (var i = 0; i <= dayshortform.length -1; i++) {
            dayshortarr.push([parseInt(i),dayshortform[i]]);
            dayfullarr.push(dayfullname[i]);
        }
		...
		function WeeklyDealView() {
            ...
            var arr = new Array();
            arr = $("#hdnWeeklyDealViewCount").val().toString().split(',');
            var maxvalue = 0;
            var minvalue = 0;
            
            for (var i = 0; i <= arr.length -1; i++) {
                weeklydealview.push([i, parseInt(arr[i])]);
                if (arr[i] > maxvalue)
                    maxvalue = parseInt(arr[i]);
                if (arr[i] < minvalue)
                    minvalue = parseInt(arr[i]);
            }
            ...
            $.plot($("#crtWeeklyDealView"), [weeklydealview], {
                series: {
                    stack: stack,
                    lines: {
                        show: lines,
            ...
            $("#crtWeeklyDealView").bind("plothover", function (event, pos, item) {
                $("#x").text(pos.x);
                $("#y").text(pos.y.toFixed(2));
                ...
                if (item) {
                    if (previousPoint != item.dataIndex) {
                        ...
                        // showTooltip(item.pageX, item.pageY, "Deal views on " + day[x] + ": " + y);
                        showTooltip(item.pageX, item.pageY, "Deal views on " + dayfullarr[x] + ": " + y);
                    }
                } else {
                    $("#tooltip").remove();
                    previousPoint = null;
        ...
        function WeeklyDealFavorite() {
            var weeklydealfavorite = [];
           
            var arr = new Array();
            arr = $("#hdnWeeklyDealFavoriteCount").val().toString().split(',');
            var maxvalue = 0;
            var minvalue = 0;
            for (var i = 0; i <= arr.length - 1; i++) {
                weeklydealfavorite.push([i, parseInt(arr[i])]);
                if (arr[i] > maxvalue)
                    maxvalue = parseInt(arr[i]);
                if (arr[i] < minvalue)
                    minvalue = parseInt(arr[i]);
            ...
            $.plot($("#crtWeeklyDealFavorite"), [weeklydealfavorite], {
                series: {
                    stack: stack,
                    lines: {
                        show: lines,
            ...
            $("#crtWeeklyDealFavorite").bind("plothover", function (event, pos, item) {
                $("#x").text(pos.x);
                $("#y").text(pos.y.toFixed(2));
                ...
                if (item) {
                    if (previousPoint != item.dataIndex) {
                        ...
                        showTooltip(item.pageX, item.pageY, "Deal favorites on " + dayfullarr[x] + ": " + y);
                    }
                } else {
                    $("#tooltip").remove();
                    previousPoint = null;
        ...
        function WeeklyDealOrders() {
            var weeklydealorder = [];

            var arr = new Array();
            arr = $("#hdnWeeklyDealOrderCount").val().toString().split(',');
            var maxvalue = 0;
            var minvalue = 0;
            for (var i = 0; i <= arr.length; i++) {
                weeklydealorder.push([i, parseInt(arr[i])]);
                if (arr[i] > maxvalue)
                    maxvalue = parseInt(arr[i]);
                if (arr[i] < minvalue)
                    minvalue = parseInt(arr[i]);
            ...
            $.plot($("#crtWeeklyOrders"), [weeklydealorder], {
                series: {
                    stack: stack,
                    lines: {
                        show: lines,
            ...
            $("#crtWeeklyOrders").bind("plothover", function (event, pos, item) {
                $("#x").text(pos.x);
                $("#y").text(pos.y.toFixed(2));
                ...
                if (item) {
                    if (previousPoint != item.dataIndex) {
                        ...
                        showTooltip(item.pageX, item.pageY, "Deal orders on " + dayfullarr[x] + ": " + y);
                    }
                } else {
                    $("#tooltip").remove();
                    previousPoint = null;
        ...
        function WeeklyNotifications() {
            var weeklynotification = [];

            var arr = new Array();
            arr = $("#hdnWeeklyNotificationCount").val().toString().split(',');
            var maxvalue = 0;
            var minvalue = 0;
            for (var i = 0; i <= arr.length; i++) {
                weeklynotification.push([i, parseInt(arr[i])]);
                if (arr[i] > maxvalue)
                    maxvalue = parseInt(arr[i]);
                if (arr[i] < minvalue)
                    minvalue = parseInt(arr[i]);
            ...
            $.plot($("#crtWeeklyNotifications"), [weeklynotification], {
                series: {
                    stack: stack,
                    lines: {
                        show: lines,
            ...
            $("#crtWeeklyNotifications").bind("plothover", function (event, pos, item) {
                $("#x").text(pos.x);
                $("#y").text(pos.y.toFixed(2));
                ...
                if (item) {
                    if (previousPoint != item.dataIndex) {
                        ...
                        showTooltip(item.pageX, item.pageY, "Notifications sent on " + dayfullarr[x] + ": " + y);
                    }
                } else {
                    $("#tooltip").remove();
                    previousPoint = null;
        ...
        function WeeklyBestDealAustralian()
        {
            var weeklynbestdealaustralian = [];

            var arr = new Array();
            arr = $("#hdnWeeklyBestDealAustralianCount").val().toString().split(',');
            var maxvalue = 0;
            var minvalue = 0;
            for (var i = 0; i <= arr.length; i++) {
                weeklynbestdealaustralian.push([i, parseInt(arr[i])]);
                if (arr[i] > maxvalue)
                    maxvalue = parseInt(arr[i]);
                if (arr[i] < minvalue)
                    minvalue = parseInt(arr[i]);
            ...
            $.plot($("#crtWeeklyBestAustralianDeal"), [weeklynbestdealaustralian], {
                series: {
                    stack: stack,
                    lines: {
                        show: lines,
            ...
            $("#crtWeeklyBestAustralianDeal").bind("plothover", function (event, pos, item) {
                ...
                if (item) {
                    if (previousPoint != item.dataIndex) {
                        ...
                        showTooltip(item.pageX, item.pageY, "Orders placed on " + dayfullarr[x] + ": " + y);
                    }
                } else {
                    $("#tooltip").remove();
                    previousPoint = null;
        ...
        function WeeklyBestDealNorwegian()
        {
            var weeklynbestdealaustralian = [];

            var arr = new Array();
            arr = $("#hdnWeeklyBestDealNorwegianCount").val().toString().split(',');
            var maxvalue = 0;
            var minvalue = 0;
            for (var i = 0; i <= arr.length; i++) {
                weeklynbestdealaustralian.push([i, parseInt(arr[i])]);
            ...
            $.plot($("#crtWeeklyBestNorwegianDeal"), [weeklynbestdealaustralian], {
                series: {
                    stack: stack,
                    lines: {
                        show: lines,
            ...
            $("#crtWeeklyBestNorwegianDeal").bind("plothover", function (event, pos, item) {
                $("#x").text(pos.x);
                $("#y").text(pos.y.toFixed(2));
        ...
        //graph  DATA_DATA Dataset 
      function graphs() {

            var graphData = [];
            var series = Math.floor(Math.random()) + 1;
        ...
        WeeklyDealOrders();
        WeeklyNotifications();
        WeeklyBestDealAustralian();
        WeeklyBestDealNorwegian();
        graphs();
    ...
    var handleFancyBox = function () {
        ...
        if (document.URL.indexOf("merchantcontactrequest_view.aspx") != -1) {
        ...
        if (document.URL.indexOf("merchantstores_addedit.aspx") != -1 || document.URL.indexOf("merchantdeals_addedit.aspx") != -1) {
            $(".fancybox-inner").next().hide();
            $(".fancybox-inner").next().next().hide();
        }
    }
    ...
    var handleLoginForm = function () {
        ...
        jQuery('#ancCanclePassword').click(function () {
            jQuery('#forgotform').hide(200);
        });
    ...
    var handleStyler = function () {
        ...
        jQuery('#theme-change .colors span').click(function () {
            ...
            setColor(color);
            $("#hdnCurrentThemeColor").val(color);
            $.ajax({
                type: "POST",
                url: "savetheme.aspx/SaveTheme",
                data: '{themename: ' + JSON.stringify($("#hdnCurrentThemeColor").val()) + '}',
    ...
    var handleToggleButtons = function () {
        ...
        $('.text-toggle-button').toggleButtons({
            ...
            //        enabled: {
            //            background: "#62c462",
            //            gradient: "-webkit-linear-gradient(bottom, #62c462, #51a351)",
            //            color: "#FFFFFF"
            //        }
            //    }
            //}
        });
        $('.text-toggle-button2').toggleButtons({
            width: 200,
            label: {
                enabled: "Active",
                disabled: "Inactive"
        ...
        $('.text-toggle-button3').toggleButtons({
            width: 100,
            label: {
                enabled: "Yes",
                disabled: "No"
    ...
    var handleTables = function () {
        ...
        //     //   $(".srno", nRow).html(iDisplayIndex +1);
        //      // return nRow;
        //  //}
    ...
    var handleDateTimePickers = function () {
		...
		$('.date-picker-report').on('changeDate', function (ev) {
            $(this).datepicker('hide');
        });
		
		
        $('.date-picker-news').on('changeDate', function (ev) {
            $(this).datepicker('hide');
        });
		 /*272*/
        

        $('.date-picker-end-filter').on('changeDate', function (ev) {
            $(this).datepicker('hide');
        });
        /*272*/
