...
public class filemanager : IHttpHandler
{
    ...
    private string getFolderInfo(string path)
    {
        ...
        //objSiteSettingService = Services.ServiceFactory.SiteSettingService;

        //System.Data.DataTable dtSiteSettings = new System.Data.DataTable();
        //objSiteSettingService.GetAllSiteSetting(ref dtSiteSettings);
        string sitepath = System.Configuration.ConfigurationManager.AppSettings["SiteAddress"].ToString();
        if (path.Contains(sitepath))
        {
            path = path.Replace(sitepath, "");
        }
        ...
        foreach (FileInfo fileInfo in RootDirInfo.GetFiles())
        {
            ...
            sb.AppendLine("\"Path\": \"" + sitepath + Path.Combine(path, fileInfo.Name) + "\",");
            sb.AppendLine("\"Filename\": \"" + fileInfo.Name + "\",");
            sb.AppendLine("\"File Type\": \"" + fileInfo.Extension.Replace(".", "") + "\",");

            if (IsImage(fileInfo))
            {
                sb.AppendLine("\"Preview\": \"" + sitepath + Path.Combine(path, fileInfo.Name) + "\",");
            }
            else
            {
    ...
    private string getInfo(string path)
    {
        ...
        //Dim objSiteSettingService As ISiteSettingService
        //objSiteSettingService = Services.ServiceFactory.SiteSettingService
        //Dim dtSiteSettings As DataTable = New DataTable()
        //objSiteSettingService.GetAllSiteSetting(dtSiteSettings)
        //If dtSiteSettings.Rows.Count > 0 Then
        //    Application("siteurl") = dtSiteSettings.Rows(0)("vSiteAddress")
        //End If
        //ISiteSettingService objSiteSettingService;
        //objSiteSettingService = Services.ServiceFactory.SiteSettingService;
        ...
        else
        {
            ...
            sb.AppendLine("{");
            sb.AppendLine("\"Path\": \"" + sitepath + path.Replace("~/", "") + "\",");
            sb.AppendLine("\"Filename\": \"" + fileInfo.Name + "\",");
            sb.AppendLine("\"File Type\": \"" + fileInfo.Extension.Replace(".", "") + "\",");

            if (IsImage(fileInfo))
            {
                sb.AppendLine("\"Preview\": \"" + sitepath + path.Replace("~/", "") + "\",");
    ...
    private string Rename(string path, string newName)
    {
        ...
        if ((attr & FileAttributes.Directory) == FileAttributes.Directory)
        {
            ...
            sb.AppendLine("\"Code\": 0,");
            sb.AppendLine("\"Old Path\": \"" + sitepath + path + "\",");
    ...
    private string AddFolder(string path, string NewFolder)
    {
        ...
        sb.AppendLine("{");
        sb.AppendLine("\"Parent\": \"" + sitepath + path + "\",");
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        switch (context.Request["mode"])
        {
            ...
            case "download":
                ...
                //objSiteSettingService.GetAllSiteSetting(ref dtSiteSettings);
                string sitepath = System.Configuration.ConfigurationManager.AppSettings["SiteAddress"].ToString();
                string pathDownload = context.Request["path"];
                pathDownload = pathDownload.Replace(sitepath, "~/");

                FileInfo fi = new FileInfo(context.Server.MapPath(pathDownload));
